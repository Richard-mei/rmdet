...
@torch.no_grad()
def retina_nms(prediction, conf_thresh=0.25, iou_thresh=0.45, classes=None, agnostic=False, multi_label=False,
               labels=(), max_det=300):
    """
    Args:
        prediction: shape(bs, sum(w*h*a), num_cls+4)  num_cls+4:
        conf_thresh:
        iou_thresh:
        classes:
        agnostic:
    ...
    for xi, x in enumerate(prediction):
        ...
        box = x[:, :4]
        if multi_label:
            i, j = (x[:, 4:] > conf_thresh).nonzero(as_tuple=False).T  # 每个框可能预测多个不同的结果！！！只要对应概率大于阈值即保留
            x = torch.cat((box[i], x[i, j + 4, None], j[:, None].float() + 1), 1)  # x[i, j + 4, None] 代表类别对应的conf，j[:, None] 代表类别  类别编号从1开始！！！
        else:
            conf, j = x[:, 4:].max(1, keepdim=True)
            x = torch.cat((box, conf, j.float()+1), 1)[conf.view(-1) > conf_thresh]  # j+1 表示预测类别 从1开始编号
        # Check shape
        n = x.shape[0]  # number of boxes
        if not n:  # no boxes
            continue
